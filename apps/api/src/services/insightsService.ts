...
export async function getInsightsData(params: {
  ...
  try {
    ...
    // Series start gives exactly `months` buckets: (now - (months-1) months) through now
    const timeseriesStartDate = new Date(now);
    timeseriesStartDate.setMonth(timeseriesStartDate.getMonth() - months + 1);
    timeseriesStartDate.setDate(1);
    ...
    const totalAwardedAmount = awardedGrants.reduce((sum: number, grant: any) => sum + Number(grant.amount), 0);
    ...
    if (departmentId) {
      medianResult = await prisma.$queryRaw<Array<{ median_days: number | null }>>`
        ...
        FROM grants
        WHERE status = 'AWARDED'::"GrantStatus"
      ...
      timeseriesResults = await prisma.$queryRaw<Array<{
        ...
        WITH month_series AS (
          ...
          FROM generate_series(
            DATE_TRUNC('month', ${timeseriesStartDate}::timestamp),
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    statusCountsByMonth.forEach((item: any) => {
      ...
      if (!statusCountsMap.has(month)) {
        statusCountsMap.set(month, {
          draft: 0, submitted: 0, under_review: 0, awarded: 0, declined: 0,
        });
      }
      const counts = statusCountsMap.get(month)!;
      counts[item.status.toLowerCase()] = item._count.id;
    });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const timeseries: InsightsTimeSeriesPoint[] = timeseriesResults.map((row: any) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const dailyActivity: DailyActivityPoint[] = dailyResults.map((row: any) => ({
      date: row.date,
      submissions: Number(row.submissions),
      awards: Number(row.awards),
      awardedAmount: row.awarded_amount ? Number(row.awarded_amount) : 0,
    ...
    sponsorGrants.forEach((grant) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const key = (grant as any).sponsor.name;
      const existing = sponsorMap.get(key) || {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        name: (grant as any).sponsor.name,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sponsorType: (grant as any).sponsor.sponsorType,
        amount: 0,
        count: 0,
      };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      existing.amount += Number((grant as any).amount);
      existing.count += 1;
    ...
    const sponsorBreakdown: SponsorBreakdownPoint[] = Array.from(sponsorMap.values())
      ...
      .map((item) => ({
        name: item.name,
        sponsorType: item.sponsorType ? item.sponsorType.toLowerCase() : null,
    ...
    const deptAwardsMap = new Map(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      deptAwards.map((item: any) => [item.departmentId, { count: item._count.id, amount: Number(item._sum.amount || 0) }])
    );
    ...
    const departments = await prisma.department.findMany({
      where: {
        id: {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          in: deptGrants.map((g: any) => g.departmentId),
        },
      },
    });
    ...
    const departmentBreakdown: DepartmentBreakdownPoint[] = deptGrants
      ...
      .map((grant: any) => {
        ...
        return {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          awardedAmount: (awards as any).amount,
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          awards: (awards as any).count,
          submissions: grant._count.id,
        };
      })
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .sort((a: any, b: any) => b.awardedAmount - a.awardedAmount);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    funnelCounts.forEach((item: any) => {
      funnelMap[item.status] = item._count.id;
    });

    const funnel: FunnelData = {
      submitted: funnelMap[GrantStatus.SUBMITTED] || 0,
      underReview: funnelMap[GrantStatus.UNDER_REVIEW] || 0,
      awarded: funnelMap[GrantStatus.AWARDED] || 0,
      declined: funnelMap[GrantStatus.DECLINED] || 0,
    };
