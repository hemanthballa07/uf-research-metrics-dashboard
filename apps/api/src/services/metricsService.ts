...
export async function getMetricsSummary(): Promise<MetricsSummary> {
  try {
    ...
    // Total dollars awarded for grants AWARDED in the last 12 months.
    // (Dollars-awarded and proposal-success-rate are intentionally different
    // metrics; they use different time windows.)
    ...
    const totalAwardedAmount = awardedGrants.reduce(
      ...
      (sum: number, grant: any) => sum + Number(grant.amount),
      0
    );

    // Award rate: of grants SUBMITTED in the last 12 months, how many are currently AWARDED?
    // Both numerator and denominator filter by submittedAt so the ratio is always ≤ 1.
    // (Previously the numerator filtered by awardedAt, producing >100% when long
    // submit→award lags meant some current awards were submitted >12 months ago.)
    const awardedFromWindowCount = await prisma.grant.count({
      where: {
        status: GrantStatus.AWARDED,
    ...
    const awardRate =
      totalSubmissions > 0 ? awardedFromWindowCount / totalSubmissions : 0;
