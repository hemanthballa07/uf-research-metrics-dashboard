...
export function GrantDrawer({ grant, isOpen, onClose }: GrantDrawerProps) {
  const [similar, setSimilar] = useState<Array<{ id: number; title: string; score: number }> | null>(
    null
  );
  const [loadingSimilar, setLoadingSimilar] = useState(false);

  // Reset when the drawer switches to a different grant (the component stays mounted).
  useEffect(() => {
    setSimilar(null);
  ...
  async function findSimilar() {
    ...
    try {
      const res = await api.getSimilarGrants(grantId, 5);
      setSimilar(res.items.map((g) => ({ id: g.id, title: g.title, score: g.score })));
    } catch {
      setSimilar([]);
    } finally {
      setLoadingSimilar(false);
        ...
        <div style={{ padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div>
              ...
              >
                {loadingSimilar ? 'Finding…' : 'Find similar grants'}
              </button>
              {similar !== null && (
                <div style={{ marginTop: '1rem' }}>
                  {similar.length === 0 ? (
                    <p style={{ color: '#666', fontSize: '0.9rem' }}>
                      No similar grants (this grant may not be embedded yet).
                    </p>
                  ) : (
                    <ul style={{ margin: 0, paddingLeft: '1.25rem' }}>
                      {similar.map((s) => (
                        <li key={s.id} style={{ marginBottom: '0.25rem', color: '#1a1a1a' }}>
                          {s.title}{' '}
