...
export function GrantsPage() {
  ...
  const departmentFilter = searchParams.get('department') ? Number(searchParams.get('department')) : undefined;
  const sponsorFilter = searchParams.get('sponsor') ? Number(searchParams.get('sponsor')) : undefined;
  const dateFrom = searchParams.get('date_from') ?? '';
  const dateTo = searchParams.get('date_to') ?? '';
  const page = Number(searchParams.get('page') ?? '1');
  const pageSize = Number(searchParams.get('pageSize') ?? '20');

  // Search is kept in local state and debounced before writing to URL
  ...
  // Helper: write a single key to URL params; resets page to 1 for filter changes
  function updateFilter(key: string, value: string | number, resetPage = true) {
  ...
  // The search value used for API calls is what's in the URL (already debounced)
  ...
  useEffect(() => {
    fetchGrants();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, pageSize, urlSearch, statusFilter, departmentFilter, sponsorFilter, dateFrom, dateTo]);
  ...
  const sortedGrants = useMemo(() => {
    ...
    return [...grants].sort((a, b) => {
      ...
      else if (sortCol === 'submittedAt') { av = a.submittedAt ?? ''; bv = b.submittedAt ?? ''; }
      else if (sortCol === 'pi') { av = a.pi?.name ?? ''; bv = b.pi?.name ?? ''; }
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // URL update (and page reset) happens via the debouncedSearch effect
              }}
          ...
          <div>
            ...
            <select
              value={sponsorFilter || ''}
              onChange={(e) => {
                updateFilter('sponsor', e.target.value);
              }}
              style={{
                width: '100%',
                padding: '0.5rem',
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const err = validateDateRange(newDateFrom, dateTo);
                setDateError(err);
                updateFilter('date_from', newDateFrom);
              }}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const err = validateDateRange(dateFrom, newDateTo);
                setDateError(err);
                updateFilter('date_to', newDateTo);
              }}
      ...
      >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f5f5f5', borderBottom: '2px solid #ddd' }}>
                ...
                >
                  Submitted{sortCol === 'submittedAt' ? (sortDir === 'asc' ? ' ▲' : ' ▼') : ' ↕'}
                </th>
              </tr>
            </thead>
            <tbody>
              {sortedGrants.map((grant) => (
                <tr
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = '#db8b8bff';
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <td style={{ padding: '1rem' }}>
                    ...
                    >
                      {grant.status.replace(/_/g, ' ').toLowerCase()}
                    </span>
                  </td>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <button
              onClick={() => updateFilter('page', Math.max(1, page - 1), false)}
              disabled={page === 1}
              style={{
                padding: '0.5rem 1rem',
                border: '1px solid #ddd',
            ...
            <button
              onClick={() => updateFilter('page', Math.min(totalPages, page + 1), false)}
