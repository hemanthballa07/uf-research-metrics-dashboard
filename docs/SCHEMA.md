...
| grantNumber | String?   | Unique, Nullable      | Stable UF Award ID (myUFL/PeopleSoft); canonical dedup key when present |
...
**Indexes:**
- `grantNumber` - Unique (`grants_grantNumber_key`); canonical idempotency key for grants that carry an Award ID (NULLS DISTINCT, so legacy rows without one are allowed)
- `(title, piId) WHERE grantNumber IS NULL` - **Partial** unique index (`grants_title_piId_legacy_key`); legacy dedup for rows without an Award ID, so two awards sharing a title+PI (e.g. competing renewals) can coexist
...
- **Grant Identity**: `grantNumber` (UF Award ID) is unique when present and is the canonical dedup key; legacy rows without it dedup via a *partial* unique index on `(title, piId) WHERE grantNumber IS NULL`, so renewals sharing a title+PI but with distinct Award IDs can coexist
